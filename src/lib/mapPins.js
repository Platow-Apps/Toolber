 ...
 *
 * Deliberately unlike a pin: a pin marks a thing that is there, and this marks
 * the person looking. Round, no tail, no label, and blue — which is what every
 * map in the world uses for this, so it needs no explaining.
 *
 * The accuracy ring is drawn at a fixed size rather than scaled to metres.
 * Doing it properly means recomputing on every zoom, and the honest version of
 * "somewhere in this circle" is not worth that on a map where every other
 * point is fuzzed by design.
 */
export function deviceDotElement() {
  const el = document.createElement("div");
  ...
  halo.style.cssText =
    "position:absolute;inset:0;border-radius:50%;background:rgba(40,120,184,.22)";
  el.appendChild(halo);

  // Set one property at a time rather than through cssText: a single
  // declaration jsdom cannot parse makes it discard the whole block, which is
  // how this first shipped as an invisible dot that still passed a smoke test.
