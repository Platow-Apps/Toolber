...
### Landing a change
`main` **is** protected by the `test` and `database` status checks (ruleset *main — require CI*, active since 16 Sep, no bypass actors, strict), which makes a direct push impossible — the checks cannot have run on a commit the remote has never seen. Two commands replace the five-step dance:
...
- **Owner identity is a *row*-level rule, and it lives in the `profiles` SELECT policy (0057).** Two things hang off it. An account is required to learn who owns a tool at all -- `display_name`, `avatar_url`, `is_platform_admin` and, the one that mattered, `tools.chest_id` are no longer granted to `anon`, because chest_id is a join key: with it, any tool led to every other tool by the same person, attached to a name, with no account. And an owner can set `identity_private`, which hides their name **and their map pin** from anyone outside their approved groups while leaving their tools searchable. **The borrow-request exception is asymmetric, and must stay that way (0059).** A lender sees a private borrower's name the moment they ask, because being asked to hand a stranger a tool and not being told who they are is approving blind. A borrower sees a private lender's name only once the lender *approves* — otherwise a stranger unmasks anyone by sending a request they then abandon. `start_conversation()` refuses against someone you cannot already see, because `authenticated` still holds the column grant on `tools.chest_id` (half the app needs it), so a signed-in stranger can read the id off the REST API and call the RPC directly however well the UI hides the button. Name and pin are one switch on purpose -- a chest's tools all sit on one stored, jittered point, so a shared coordinate identifies an owner as well as a label does, and hiding one without the other is theatre. Don't add a second switch that separates them. Because the rule depends on the *pair* of people, a column grant cannot express it; putting it in the policy is what makes every PostgREST embed, RPC and hand-written query get it for free. **There are two predicates, not one, and conflating them is a shipped bug.** `owner_identity_visible()` gates the name, the avatar and `chest_id`, and is false for anyone signed out. `owner_pin_visible()` gates the map coordinates and the distance ordering, and is true for everyone unless the owner went private — because a pin is a fuzzed point that was never anyone's real position, and the map is the product. 0057 hung the pin off the identity predicate and emptied the public map for every logged-out visitor; 0058 split them. A privilege check cannot see that class of mistake, so the self-check and the pgTAP suite both *run a search as anon* and assert a pin comes back. Two more consequences to remember: `search_tools()` had to become SECURITY DEFINER (reversing 0042's note) since anon may no longer select the columns it joins on, so its return list is now the only thing standing between it and a leak -- keep the self-check honest; and a private owner's profile row is simply *absent* for a stranger, so read it with `.maybeSingle()`, never `.single()`, which turns a legitimate answer into a REST error on screen.
- **The admin console reads through RPCs, never through grants (0060), and `src/pages/Admin.jsx` must keep doing so.** `home_lat`, `home_lng` and `phone` are selectable by no database role, and the console did not change that — it asks `admin_user_detail()`, which checks the flag and writes an `admin_viewed_user` events row naming the admin and the person they opened *before* returning anything. Granting those columns to admins instead would work identically on screen, apply to every row for as long as the flag is set, and leave no trace of a single lookup. A direct table query in that page would quietly undo both properties, which is why `scripts/test-no-direct-pickup-location.sh` now grants Admin.jsx a *conditional* exemption: it may name `home_lat`/`home_lng`, but only while it contains no table query at all. Do not resolve a failure there by renaming the RPC's returned fields to slip past the grep — the exemption is meant to stay visible. Note also that EXECUTE on every `admin_*` function is granted to plain `authenticated` on purpose: the refusal lives in the function body, so a privilege test proves nothing and `admin_console_test.sql` calls each one as an ordinary account instead. Deletion has two shapes and they are not interchangeable — `admin_scrub_account()` wipes the identifying fields, removes their content and hands any group they ran to its longest-standing member while *keeping the row*, so a counterparty's borrow history still resolves to somebody; `admin_hard_delete_account()` removes the `auth.users` row and cascades, which tears rows out of the records of people who did nothing wrong. Both return the tool photo paths because Postgres cannot see the Storage bucket, and a caller that ignores the return value orphans every image. **Admin messaging (0061) is its own function for the same reason the console is.** `admin_message_users()` exists rather than a loop over `start_conversation()` because 0059 makes that one refuse against anybody whose identity the caller cannot already see — usually true of an admin and a reported user — and widening it for admins would widen it for the unmasking case it was written to block. It also sets `conversation_messages.from_admin`, which nothing else may: `display_name` is user-editable, so a "Toolber Admin" badge derived from a name could be imitated by anyone who renamed their account. The insert policy now requires `from_admin = false`, because a column-level revoke would be a no-op against 0040's table-level INSERT grant — the policy is the enforcement, so check it by attempting the write as a real role rather than by inspecting privileges.
- **A tool plots where it is *kept*, not where its owner lives (0063).** A profile can name places — a cabin, a shop — and `tools.location_id` points at one; `search_tools()` and `tool_owner_card()` coalesce that place's pin over the chest's, so a tool with no place behaves exactly as before. `profile_locations` holds real coordinates and a street address and therefore carries **no SELECT grant for any role**, not even its owner: every read and write is a function call. That is deliberately a stronger shape than the column-grant dance protecting `profiles` — there is no column list here for somebody to extend by accident. The jitter rule now lives in one place, `jitter_point()`, shared by `set_my_area()` and `save_my_location()`; `save_my_location()` re-jitters **only when the address or radius actually changed**, because a pin rerolled on every save lets repeated saves average out to the real address, which is the whole reason the jitter is stored rather than computed on read.
- **Replacing a function means replacing the signature it actually has.** Rewriting `set_my_area` to use the shared helper was first written against its pre-0050 three-argument shape. That does not edit the live function — it creates a *second overload* beside it, one that skips the address-certification check, and leaves a three-named-argument call ambiguous between the two. `named_locations_test.sql` now asserts exactly one `set_my_area` exists and that it still records the certification. Check `pg_get_function_arguments` before `create or replace`, not after.
- **A new table starts out fully granted, so REVOKE before you GRANT (0062).** Supabase ships default privileges handing `anon` and `authenticated` everything on a new table in `public`. A migration that adds `grant select` to a new table therefore restricts nothing — it reads as a lock-down and is a no-op, which is the table-level twin of the column-grant trap. `group_tool_requests` was written that way and its own self-check caught it with INSERT already in every signed-in client's hands. The working shape is `revoke all on t from public, anon, authenticated;` first, then the explicit grant.
- **A notification type not in the notify function's `TYPE_TO_PREFERENCE` sends nothing at all.** SEC-5 made that path fail closed on purpose — emailing someone who opted out is unrecoverable, refusing to send is not — but it means a type added in a migration and never mapped is silently dead. `tool_removed` and `group_handover` (0060) shipped that way and went unnoticed until 0062. Adding a notification type means touching three places: the migration that inserts it, `TYPE_TO_PREFERENCE` plus the copy maps in `supabase/functions/notify/index.ts`, and `src/lib/notifications.js` for the in-app bell. **And the Edge Function needs redeploying** — `npm run supabase:db:push` does not carry it.
...
- **`supabase/tests/column_grants_test.sql` is the guard against the SELECT half, and it exists because the same mistake was made six times.** Each of `tools`, `profiles`, `groups` and `borrow_requests` asserts the exact set of columns withheld on purpose, so a newly added column fails there, by name, at the point it is added. The sixth instance was `tools.general_location`: 0033 added it beside `reveal_exact_location` and granted only the second, which meant *editing any tool* failed with "permission denied for table tools" — a whole row refused for one column. It presented as an edit form with every field blank and no photos, as if editing meant starting the listing over, with the error at the bottom above a disabled Save. Nobody noticed for twenty migrations because only the edit form reads that column. Adding a column now means adding it to a grant or to that test's withheld list with a reason; both are deliberate.
...
- **An RLS policy that queries another RLS-protected table will recurse, and the gate chain cannot see it.** 0052 gave `groups` a SELECT policy asking whether the viewer is an approved member; `group_memberships` has asked since 0001 whether the viewer administers the group. Reading either evaluates the other's policy, and Postgres stops it with *"infinite recursion detected in policy for relation groups"* — which took out every signed-in read of `groups`: the Groups screen, Group Detail, the map's group pins and the shared-groups trust signal. `npm run test:all` mocks Supabase and never evaluates a policy, and the migration's own self-check asserted grants rather than running a query, so both were green. Look up the other table through a `SECURITY DEFINER` function instead (`is_approved_group_member`, 0055): it executes as the owner, who is not subject to the policies, so the cycle breaks at the first hop. Such a helper **must** be executable by `authenticated` — a policy runs as the querying role, so a role that cannot execute it cannot read the table at all. And make an RLS migration's self-check *run a query* under `set local role authenticated`, not just check privileges.
...
- **A migration's self-check runs as a different role locally than it does against the live project, so one that writes rows is not the same test in the two places.** 0064's first self-check created an account, a group and a reply to prove the repaired function returned them. It passed `supabase db reset`, it passed CI's `database` job, and `supabase db push` then failed on it with *"permission denied for table groups"* — locally the migration runs as the table owner, remotely as the CLI's login role. The push is transactional, so nothing was applied and the fix did not reach production on the first attempt; the failure is loud, but it arrives at the worst moment, after the merge, with `main` already deployed. Write self-checks that read and call rather than insert. 0064's rewritten one asks the function for an id that matches nothing: the ambiguity it guards against is resolved when the statement is planned, not when it finds a row, so a call that returns nothing still distinguishes the broken function (42702) from the repaired one (its own `P0001 No such request`) — no fixtures, no role dependency, and it was checked in both directions rather than assumed. Where a check genuinely must write, the pgTAP suite is the place for it: it runs as `postgres` by design and rolls back.
...
- **CI and your machine must run the same Node, and `.nvmrc` is where that is decided.** They drifted once — CI on 22, development on 24 — and it cost a green local run against a red CI run on the same commit. `global-jsdom` replaces `globalThis.crypto` with jsdom's, which has `getRandomValues` and no `SubtleCrypto`; on Node 24 that assignment silently fails because the global is locked, so webcrypto survives, and on Node 22 it succeeds and `subtle` disappears. `hashFile` then returns null, photo de-duplication never fires, and the only symptom is a test asserting a notice that cannot appear. `test/support/polyfills.js` restores webcrypto, and the workflow reads `node-version-file: .nvmrc` so there is one version to change rather than two to keep in step.

- **Pin the Supabase CLI, and suspect it before your own diff.** It is now an exact-versioned devDependency (`supabase: "2.116.0"`, no caret) and the scripts call the local binary, so nothing resolves it at run time. Before that they called plain `supabase` through `npx`, which fetched whatever was newest at the moment anyone ran them — 2.115.0 shipped a bundler bug that failed `functions deploy` with nothing but "An error occurred in Effect.tryPromise" after "Bundling Function" — no message, no stack. The same source deploys cleanly on 2.116.0, which the scripts now pin. The tell that it was not the code: deploying an **untouched** second function fails identically. Run that control early — one command settles "my change" versus "the tool" outright, and skipping it cost about an hour of searching a clean diff.

  Two things followed from the npx version. A bad 2.115.0 release broke `functions deploy` (below), and every invocation re-resolved the CLI through the npm cache — which corrupted twice, failing the deploy path with an ENOENT on a `_cacache` file and nothing to do with the project. `npm cache verify` clears that; the devDependency stops it reaching the deploy at all.
...
**pgTAP suite** — `supabase/tests/*.sql`, run with `supabase db reset && supabase test db` (needs Docker; there is deliberately no npm script for it, since it cannot run without a local stack). **CI runs it too, as a separate `database` job**, so it is a gate rather than a habit — it had been written on 20 Aug, first run on 3 Sep, and until then ran only when somebody remembered. That gap is not theoretical: 0052's groups policy recursed and took out every signed-in read of `groups` in production while CI stayed green throughout. The job also re-runs every migration's own self-check against a scratch build, which is how the anon-EXECUTE gap and the default-privileges INSERT leak were both caught. The AVA suite mocks Supabase and therefore cannot test a single RLS policy, column grant, or RPC guard; that is entirely what these files are for. `pickup_location_rls_test.sql` proves the pickup/home-coordinate boundary; `borrow_and_group_rls_test.sql` covers the row policies and RPC authorization. Assertions inside its `todo_start`/`todo_end` block encode intended behaviour that the schema does not have yet (audit findings PRIV-1, PRIV-2, DOS-1) — unwrap each as it is fixed.
...
## Deployment
- `.github/workflows/ci.yml` runs two jobs in parallel on every push and PR. **`test`** is the full gate chain plus a build, and needs three repo secrets: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_MAPBOX_TOKEN`. **`database`** starts a local Supabase stack, applies every migration to a scratch database and runs the pgTAP suite; it needs no secrets. Both must pass — set branch protection to require both, or a policy regression still merges.
- **`supabase/config.toml` is required and is now committed.** Until 16 Sep there was none: the CLI had been falling back to implicit defaults derived from the folder name, which happened to work on one machine and is not something CI can rely on. It pins `project_id` and `major_version = 17` (Postgres 17.6 locally), so the database CI job builds the same database a developer does. `supabase/.branches` and `supabase/.temp` are machine-local and are now gitignored rather than tracked.
