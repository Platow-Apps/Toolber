...
- [x] **Tagline, next to the wordmark, right-aligned, no dash**: cycles once through 11 phrases (6s each) — "Why buy? Borrow.", "Put your tools to work.", "Neighbors helping neighbors", "Lawn & Garden", "Power Tools", "Automotive", "Carpentry", "Industrial", "Fasteners & adhesives", "Saving $$ for thousands", "Contribute to your community" — then fades away for good, not a continuous loop. Color matches the wordmark. Respects reduced-motion. **Built for real** in `src/components/SearchTagline.jsx`, live on Search's header via `BrandBar`'s optional middle slot — an earlier, non-spec-matching 3-phrase version had drifted onto the Login/Signup `AuthHero` mid-session; that's been removed.
