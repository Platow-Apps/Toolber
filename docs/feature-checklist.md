...
## Your area is editable (2026-09-03)
- [x] **Settings → Your area** re-geocodes an address and re-derives the map pin (`0045_set_my_area.sql`). It was captured once at onboarding and never again, so a move, a typo, or a wrong neighborhood was unfixable from inside the app — and that point is the origin for proximity search, the map pin, and Find a Group.
- [x] **The jitter moved from the browser into Postgres.** It was client-side in `Onboarding.jsx`; adding a second entry point would have meant a second copy, and the fuzzing *is* the privacy model — a copy that drops the sqrt or rounds differently publishes addresses. `set_my_area()` takes a geocoded point and does the fuzzing itself, so skipping it is not the client's to do. Onboarding now calls the same RPC.
- [x] **Changing your area refreshes the pin of every group you're an approved member of.** A group's pin is the average of its members' approximate points, and `refresh_group_pin()` is deliberately not client-executable — doing both in one function is the only way they stay in step. Groups with a stated area are left alone, as 0037 established.
...
- [x] **An owner can keep their name and pin inside their groups** (`profiles.identity_private`, off by default). Outside their approved groups their tools stay searchable with no name and no pin attached. Disclosure waits for the owner's own act: a borrower learns who the lender is when the lender **approves**, not when they ask (0059) — otherwise a stranger unmasks anyone by sending a request and abandoning it. The reverse does not wait, because a lender deciding on a request needs to know who is asking.
...
- [x] **People is a table with tick-boxes, bulk scrub/hard-delete, and CSV export.** Bulk runs one call per account rather than a set-based RPC, so it reuses the tested single-account path and can report partial failure — a set-based function would refuse a whole batch because one member happened to be another admin. The confirmation panel *names* everyone selected rather than counting them: a count tells an admin nothing about whether the right rows are ticked. A search that changes the visible rows drops any selection that scrolled out of scope, so a bulk action can never reach an account the admin cannot currently see.
- [x] **The CSV carries the columns on screen and no others.** Phone numbers and home coordinates stay behind the logged RPC; a spreadsheet of them in a downloads folder is the exact shape this console exists to avoid. `src/lib/csv.js` is a pure module because the escaping is the part that breaks silently — a name like `Jim B., Jr.` splits into two columns and the spreadsheet opens without complaint.
- [x] **No home address is stored, and the console now says so.** `set_my_area()` geocodes in the browser and sends only coordinates, so the row previously labelled "Home address" was showing `default_pickup_location` — set only when somebody saves a pickup address. Blank on every account read as a bug and was a fact.
- [x] **The invariant guard covers this page** (`scripts/test-no-direct-pickup-location.sh`). Admin.jsx is the one file allowed to name `home_lat`/`home_lng`, and only while it contains no table query at all — checked, not trusted. An exemption you can see beats renaming the fields to slip past the grep.
- [x] **Message selected** (`0061_admin_messaging.sql`). One private conversation per recipient, never a group thread — a note to twelve people is twelve conversations, not a room where they read each other's business. It lands in the ordinary inbox, so a reply goes straight back and there is no second messaging system to keep.
- [x] **"From Toolber Admin" is a server-set fact, not a display name.** `display_name` is user-editable, so a badge derived from it would be worth nothing exactly when it mattered — anyone could rename themselves Toolber Admin and ask a neighbour to confirm their address. `conversation_messages.from_admin` is written only by `admin_message_users()`, and the ordinary insert policy now requires `from_admin = false`, which is what stops a client hand-inserting one (a column-level revoke would have been a no-op against 0040's table-level INSERT grant).
