...
**More than one place (0063).** A profile may name places — a cabin, a workshop — each with its own private `home_lat/home_lng` and its own public pin, jittered once and persisted under exactly the rules above. A tool's `location_id` names which place it is kept at, and `search_tools()`/`tool_owner_card()` coalesce that place's pin over the chest's, so a tool with no place is unchanged. This closes the gap that previously forced a second account: a tool's *pickup address* has been per-tool since 0001, so the handover always worked, but discovery did not — the cabin's chainsaw plotted at the owner's house and sorted by distance from there. `profile_locations` is function-only, with no SELECT grant for any role including its owner, because it holds both real coordinates and a street address.

**Who may see a pin at all, and whose it is (0057).** Two rules sit on top of the generation described above, and both are row-level rather than column-level, because they depend on the pair of people involved rather than on the column.

1. **An account is required to learn who owns a tool.** A logged-out visitor keeps the tools and the map, and loses `display_name`, `avatar_url`, `is_platform_admin` and `tools.chest_id`. That last one is the substantive change: `chest_id` is a join key, so with it any single tool led to every other tool by the same owner, attached to a name, with no account and no rate limit. `search_tools()` therefore had to become SECURITY DEFINER — it joins on columns anon may no longer select — and blanks the owner fields itself.
2. **`profiles.identity_private`** lets an owner hide their name **and their pin** from anyone outside their approved groups, while leaving their tools searchable. The two are deliberately one switch. Every tool in a chest sits on that single stored, jittered point, so a shared coordinate identifies an owner exactly as well as a label does; hiding the name and leaving the pin would look like privacy and provide none. The borrow exception runs one way only (0059): a **lender** sees a private borrower's name as soon as they ask, because deciding whether to hand a stranger a tool without being told who they are is approving blind; a **borrower** sees a private lender's name only once that lender approves, or a stranger could unmask anyone by sending a request and abandoning it. `start_conversation()` refuses against somebody you are not already entitled to see — `authenticated` keeps the column grant on `tools.chest_id` because half the app needs it, so a signed-in stranger can always read the id straight off the REST API no matter what the UI offers.

**Identity and pin are separate predicates (0058).** `owner_identity_visible()` gates the name, avatar and `chest_id` and requires an account. `owner_pin_visible()` gates the coordinates and the distance ordering, and requires only that the owner has not gone private — a pin is a deliberately fuzzed point that was never anyone's real position, and a logged-out visitor is exactly who the map is for. 0057 gated both on the identity predicate and shipped a search page with no pins on it at all; a grant audit cannot catch that, which is why the migration self-check and `owner_identity_test.sql` both run a search **as anon** and assert a pin comes back.

Enforcement is the `profiles` SELECT policy, so a private owner's row is simply absent for a stranger: PostgREST embeds return null, the chest page finds nobody, and `search_tools()` left-joins so the *tools* survive. Read that row with `.maybeSingle()`; `.single()` turns a legitimate answer into an error on screen.
